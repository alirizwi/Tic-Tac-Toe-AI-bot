...
class Team47():
    ...
    def is_good_terminal(self, blocks):
        ...
        # diamond
        if blocks[5] == blocks[8] == blocks[10] == blocks[13] == flag:
            self.good_terminal = True
            return True
        if blocks[6] == blocks[9] == blocks[14] == blocks[11] == flag:
            self.good_terminal = True
            return True
        if blocks[1] == blocks[4] == blocks[9] == blocks[6] == flag:
            self.good_terminal = True
            return True
        if blocks[2] == blocks[5] == blocks[10] == blocks[7] == flag:
            self.good_terminal = True
            return True
